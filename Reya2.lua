local UIsuccess, WindUI = pcall(function()
    local Version = "1.6.53" -- Versi WindUI yang umum digunakan di skrip asli
    local url = "https://github.com/Footagesus/WindUI/releases/download/" .. Version .. "/main.lua"
    print("[Reya HUB] Mencoba memuat WindUI dari link versi...")
    return loadstring(game:HttpGet(url))()
end)

if not UIsuccess or not WindUI then
    warn("[Reya HUB] ⚠️ Gagal memuat WindUI dari link versi. Mencoba link mentah (raw)...")
    
    local rawUrl = "https://raw.githubusercontent.com/Footagesus/WindUI/main/dist/main.lua"
    local rawSuccess, rawWindUI = pcall(function()
        return loadstring(game:HttpGet(rawUrl))()
    end)
    
    if not rawSuccess or not rawWindUI then
        error("[Reya HUB] ❌ Gagal memuat WindUI dari kedua sumber. Executor mungkin tidak stabil atau link sudah mati.")
        return
    end
    WindUI = rawWindUI
    print("[Reya HUB] ✅ Berhasil memuat WindUI dari link mentah.")
end
...
-- Kode invite Discord (Placeholder: reyahub)
